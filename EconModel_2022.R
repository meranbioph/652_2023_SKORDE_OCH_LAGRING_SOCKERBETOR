...
kr_marknad_90 <- 1.80*sek_eur # source: "Information kring kontrakteringen", på sockerbetor.nu 20 augusti 2021
kr_marknad_80 <- 1.40*sek_eur # source: "Information kring kontrakteringen", på sockerbetor.nu 20 augusti 2021
...
temp_tab_historical <- data.frame(
...
### LOCATIONS
stations <- data.frame("WSTNID" = c(20947, 36010, 40007, 40141, 40143, 40144, 40145, 41105), # NBRs stations = 40141, 40142, 40143, 40144, 40145, 
                       "WSTN_namn" = c("Borgeby","Hammenhög","Sandby gård","Gretelund","Lovisero","Tofta","Hviderup", "Jonstorp"), 
                       "lat" = c(55.7530, 55.48809, 55.4395, 55.8864, 55.3772, 55.8935, 55.7779, 56.2295), 
                       "long" = c(13.0396, 14.19498, 14.1827, 14.0355, 13.4048, 12.9146, 13.3217, 12.6535))
...
stations_c <- rep(stations$WSTNID, each=len_matrix/len_stations)
...
for (i in 1:len_matrix){
  ...
  dat_in_i$HOUR <- as.numeric(dat_in_i$HOUR)
